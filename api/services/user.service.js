...
const client = require("twilio")(Env.TWILIO_ACCOUNTSID, Env.TWILIO_AUTHTOKEN);
...
exports.addReferral = async function (obj) {
  ...
  try {
    ...
    if (fromUserVerified && toUserVerified) {
      ...
      return status;
    } else if (fromUserVerified && !toUserVerified) {
...
exports.getAllUsers = async function (obj) {
  ...
  let users = await User.find({
    $and: [{ email: { $ne: obj.email } }, { walletAddress: { $ne: null } }],
  });
...
exports.contactUs = async function (obj, user) {
  ...
  emailUtils.sendContactUsEmailSG(
    user.name,
    obj.contact_type,
    user.phoneNumber,
    user.email,
