...
exports.me = async function (req, admin) {
  ...
  let match = {
    createdAt: { $gte: new Date(todayStartDate), $lte: new Date(todayEndDate) },
  };
  let todayTotalTransaction = await EthereumService.findAndCountAllTransaction(
    match
  );
  return {
    admin: admin,
...
exports.login = async function (obj) {
  ...
  if (!(await passwordUtils.verifyPassword(admin, obj.password)))
